...
interface CodeSuggestionsRequest {
  fileContent: string;
  cursorLine: number;
  cursorColumn: number;
...
export async function POST(request: NextRequest) {
  try {
    const body: CodeSuggestionsRequest = await request.json()
...
5. Forget the before suggestion and only focus on the current line
...
async function generateSuggestion(prompt: string): Promise<string> {
  try {
    ...
    if (!response.ok) {
      ...
      return `// AI suggestion unavailable (Error: ${response.statusText})
// Please ensure Ollama is running with codellama model installed`
    }
    ...
    // Return a more helpful error message
    return `// AI suggestion unavailable (${error instanceof Error ? error.message : 'Unknown error'})
// Please ensure Ollama is running with codellama model installed`
  }
}
